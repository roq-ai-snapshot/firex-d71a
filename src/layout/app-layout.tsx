...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['fire-chief'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<DepartmentInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/departments` : null),
    () => getDepartments({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Department',
      path: isTenantUser ? `/departments/view/${data?.[0]?.id}` : '/departments',
      entity: 'department',
