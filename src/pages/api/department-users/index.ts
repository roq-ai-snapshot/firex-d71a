...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  switch (req.method) {
    ...
    case 'POST':
      return createDepartmentUser();
    default:
      return res.status(405).json({ message: `Method ${req.method} not allowed` });
  }

  async function getDepartmentUsers() {
    const data = await prisma.department_user
      .withAuthorization({
        roqUserId,
        tenantId: user.tenantId,
        roles: user.roles,
      })
      .findMany(convertQueryToPrismaUtil(req.query, 'department_user'));
  ...
  async function createDepartmentUser() {
    ...
    const data = await prisma.department_user.create({
      data: body,
    });
    return res.status(200).json(data);
  }
